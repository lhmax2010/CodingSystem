...
**版本**: v0.4 Draft（v0.3 三方复审 1 GO / 2 NO-GO 后修订版，待第四轮复审）
**状态**: 复审中
**读者**: 框架开发（Codex）、cross-reviewer（Kimi / Codex / Claude Code）、后续业务 Agent 开发者
**关联文档**: 01_Contract_Spec、02–06_Agent_Design_Guide、07_Conformance_and_Onboarding、08_Codex_Dev_Guide、09_Phased_Dev_Plan、docs/review/HLD_v0.1/v0.2/v0.3_review_disposition.md（v0.3 处置表为本版逐条依据）
...
- 陈旧性：与当前最新 series 不一致的到达结果标记 stale 丢弃、继续等待同 series 新结果；被取代 series 的在飞任务跑完即弃（MVP 不主动取消）；join 等待受 `timeout_min`（可选）约束、缺省以 run_budget 为界，计时自该 activation 首个到达起算，超时 → fail_safe。
...
```
created ─▶ awaiting_patch ─▶ queued ─▶ running ◀─resume─ paused(gate)
   │            │(TTL 24h)      ▲        │  ▲               │(reject/TTL)
   │(snapshot   ▼               └─(槽空闲)┘  └─(节点流转)     ▼
   │ 失败)   closed(cancelled)           ▼            （经 fail_safe 节点）
   ▼                              closed(succeeded)   closed(fail_safe)
 closed(snapshot_error)           closed(fail_safe)   /  publishing ─▶ closed(...)
                                  closed(cancelled)
```
- 状态穷尽：created / awaiting_patch（snapshot 已 pin、等待 submit_patch，**不占执行队列**）/ queued（patch 受理、FIFO 排队）/ running / paused(gate) / **publishing**（授权已核销、发布 point-of-no-return，§8.3）/ closed(reason)，reason ∈ {succeeded, fail_safe, cancelled, snapshot_error}。全部转换的命令/guard 表 `[01]`；非法转换由 Ledger 状态机守卫拒绝并记审计。
...
- `sys.*` 默认走 `default_route`（即 fail_safe）；节点可配置 `retry: { on: [sys.crash, sys.error, sys.timeout], max: N }`（timeout 重试的重复副作用由幂等层覆盖）由编排层以同 task 新 attempt 重试——**幂等键不含 attempt**，重试命中同一效应记录（§5.2）。
...
- `idem_key = hash(run_id, node, loop_round, effect_class, effect_params_digest, effect_seq)`——**效应身份锚定在效应级而非 task 级**：`effect_params_digest` = side_effect 调用实参的规范化摘要（含所操作的 series_id/tree_digest、构建目标、板卡标识等，artifact 引用按 content digest 参与），`effect_seq` = 同 task 内实参完全相同的连续调用的单调序（定义 `[01]`）。**不含 attempt**：参数相同的重试/重放命中同一记录；非决定论重执行（如 LLM 产出不同 patch）实参不同 → 新键，**不会**错误复用旧效应结果。attempt 启动前，编排层将上一 attempt 遗留的本 task 非终态效应记录逐条 probe 对账后标记 abandoned（成功者保留为已完成但不复用），防孤儿累积。
...
- **task 级绑定**：worker 的 socket FD 由编排进程 spawn 时注入（绑定身份），服务端从连接上下文推导 run_id/task_id 并以 Ledger task 租约校验——客户端不得自报归属，越域请求（为他 task 提交 side_effect/trace 等）拒绝并记审计；conformance 含跨 task 篡改拒绝用例（§11.3）。
...
- A3 引用：01/07/08/09（待产出）；docs/review/ 下 v0.1–v0.3 三轮处置表。
