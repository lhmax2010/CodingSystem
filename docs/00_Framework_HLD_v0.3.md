...
**关联文档**: 01_Contract_Spec、02–06_Agent_Design_Guide、07_Conformance_and_Onboarding、08_Codex_Dev_Guide、09_Phased_Dev_Plan、docs/review/HLD_v0.1_review_disposition.md、docs/review/HLD_v0.2_review_disposition.md（本版逐条处置表）
...
| D12 | 执行环境固化 | Run 创建时固化 execution manifest：pipeline digest、contract 版本、框架版本、agent 制品 digest、provider 配置 digest、引擎版本；task dispatch / 审批 / 发布前复核相关 digest；恢复时不匹配走 ledger 级 administrative finalize（§4.3.3），不做在途迁移；provider 配置 digest 仅含策略语义（不含端点/凭据，轮换不影响恢复） | 官方明示 interrupted thread 不支持拓扑变更（节点改名/删除）、state 不兼容变更有风险；逐类推理迁移安全性的成本与出错面大于一律拒绝 |
...
| **approval token / 授权记录** | approve 时在 Ledger 生成的一次性授权记录（token_id + release_manifest_digest：含 snapshot、tree_digest、发布目标、动作、pipeline_digest、exec_manifest_digest、过期时间）；不以 bearer 形态进入图状态/checkpoint；仅由 broker 经特权 RPC 原子核销一次；series 变化/Run 关闭/过期即失效 |
...
- 保留节点见 §3；`entry` 必填且唯一；终局节点为 `release`、`fail_safe`、或显式 `terminal: true` 节点（子集流水线用）。terminal 节点必须声明 `success_on: [status...]`：outcome ∈ success_on → closed(succeeded)，其余（含 sys.*）→ fail_safe；`compiler_bench_only.yaml` 示例按此交付。release 保留节点的 outcome 集与终态映射固定（§4.2.5）。
...
#### 4.2.4 并行与汇合
- fan-out：`to: [a, b]` 并行分派。
- **join 触发机制（定案）**：join 的"到达" = 指向该 join 的路由边送达事件；校验器强制 `wait_for` 集合 ≡ 指向该 join 的路由源集合（不一致拒绝加载），语义唯一无歧义。all-of：全部源到达且 **series_id 一致**才触发；触发后下游 TaskInput 以固定形态 `join_inputs: list[上游结果引用]` 携带全部输入（schema `[01]`）。
- 陈旧性：与当前最新 series 不一致的到达结果标记 stale 丢弃、继续等待同 series 新结果；被取代 series 的在飞任务跑完即弃（MVP 不主动取消）；join 等待受 `timeout_min`（可选）约束、缺省以 run_budget 为界，超时 → fail_safe。
- 并行分支任一进入 fail_safe：编排层取消同 Run 其余在飞分支（worker 终止，§5.1.2），Run 走终局处理。
- 通则：任何 Report 携带 series_id；**series_id 由 SDK 原子 API 向框架登记签发**（Ledger 计算 tree_digest），HandoffResult 中的 series_id 经框架校验、不可自造；series 变化自动使未核销授权失效（§8.2）。

#### 4.2.5 特权节点
- `broker: release` 节点由框架内置实现（转发 ReleasePlan 至 release broker），不可被业务 agent_type 顶替。release 的 outcome 集固定为 {released, release_failed}（授权无效/过期/digest 不符归入 release_failed 附 reason）：released → closed(succeeded)，release_failed 走路由（默认 fail_safe）。校验器强制：所有到 release 的路径必经 hitl，且 **hitl 的 approved 出边必须直达 release**——审批与执行之间不得存在任何可改变 release manifest 的节点；凭据只在 broker 进程，业务 CI Agent 永远无凭据，"CI 类节点判定"问题不存在。

#### 4.2.6 子集合法性（"合法子集"的定义）
静态校验器全部规则：节点/路由/join 引用有效；entry 存在；每个可达环均被某 `loops` 声明覆盖且带 budget；路由穷尽（4.2.3）；含 release 节点则必经 hitl 且 approved 出边直达 release（4.2.5）；join 的 wait_for ≡ 指向该 join 的路由源集合（4.2.4）；hard-required 上游做**支配性检查**——到消费节点的每条路径都必须先经过生产节点，仅存在不满足；terminal 节点必有 success_on；threshold gate 引用的指标来源节点在图中存在（不存在即拒绝加载——hard-required 依赖同理：Agent contract 可声明少数 hard-required 上游产物 `[01]`，其余一律 optional 降级，§6.3）；无不可达节点。通过全部规则 = 合法子集；示例 `pipelines/compiler_bench_only.yaml` 随仓交付。
...
- `snapshot_pinned` 等待 submit_patch 有 TTL（配置，默认 24h），超时 closed(cancelled)；run_budget 自 submit_patch 受理起算。
- 执行槽语义（D11）：paused 不占执行槽；resume 与新 Run 同入 FIFO 执行队列（§6.5）。
...
- `sys.*` 默认走 `default_route`（即 fail_safe）；节点可配置 `retry: { on: [sys.crash, sys.error, sys.timeout], max: N }`（timeout 重试的重复副作用由幂等层覆盖） 由编排层重试（同 task 新 attempt，幂等键含 attempt 序号的意图记录见 §5.2）。
...
- 运行期完整性：task dispatch / 审批 / broker 发布前复核相关制品与配置 digest（D12），不匹配同走 administrative finalize。
...
- `hitl`：gate 等待为**节点级**——同 Run 其他在飞分支继续执行；Run 级 paused 当且仅当仅剩 gate 等待、无可运行工作（此时释放执行槽，D11）。Approval 服务的决定经 control-plane RPC 单命令原子提交（决定 + 授权记录 + gate outcome + resume outbox，§8.5），编排层据 outbox 触发 resume；**resume 不消费授权**，唯一消费点在 broker（§8.3）。gate 可配置 TTL（默认无），超时 outcome=gate_expired；等待期间 series 变化按 §4.2.4 陈旧性规则处理。
- `threshold`：纯代码判定；指标缺失/非法一律 fail-closed（outcome=fail），不允许 fail-open。

### 4.5 循环预算
- 预算属于 `loops` 声明的环对象（环由 `edges` 唯一定义，routes 不重复标注环归属）；环边每通过一次计数 +1，**max_rounds: N = 环边允许通过 N 次，第 N+1 次通过请求即拦截并强制路由 fail_safe**（示例 max_rounds: 2 即最多 2 轮修复回流）；计数持久化于 Ledger 随恢复延续；wall_clock 排除 paused 时段；多环共享节点各环独立计数。`run_budget` 为 Run 级总预算，最先触发者生效。
...
- 编排层持有 worker 租约（heartbeat）；**worker 侧心跳丢失即自杀**（超时自行退出，副作用由 §5.2 probe 对账）；编排进程恢复时先按 Ledger 登记的 pgid 扫描 reap 孤儿进程组、再重放。
...
- `idem_key = hash(run_id, node, loop_round, effect_seq, input_digest)`——**不含 attempt**：重试/重放针对同一逻辑副作用，必须命中前一 attempt 的记录（attempt 只进 trace）。同键存在 claimed/running 记录时先 probe 对账：效应已成功 → probe 返回结果描述符，补写完成记录并复用结果；未发生 → 续用该记录重新执行。`input_digest` = TaskInput 规范化摘要，artifact 引用按 **content digest** 参与（不可变性 §7.5）。
...
- 操作：`create_run(pipeline_id, target_spec)`（target_spec 指定 snapshot 拉取范围：仓/包/manifest，语义 `[01]`）、`submit_patch(run_id, series)`（unified diff series + 逐 patch 的 repo 归属；服务端校验对 pinned snapshot 可应用，不可应用即拒绝；一个 Run 仅接受一次初始 series，重复提交返回明确错误——修订走新 Run）、`get_status`、`get_result`、`cancel_run`。忙时语义：已有执行中 Run 时 create_run 受理但入 FIFO 执行队列（响应返回队列位置），resume 同队列；awaiting-patch TTL 见 §4.3.1。
...
- **Blob 提交协议**：artifact 落盘顺序强制 = 临时文件写入 → fsync → rename 至 digest 路径 → Ledger 索引事务提交；崩溃孤儿 blob 定期对账 GC；索引存在而 blob 缺失 = 损坏，消费方报 sys.error。
...
- approve 经 control-plane RPC 单命令原子提交：审批决定 + **Ledger 授权记录（token_id + manifest digest + 过期时间）** + gate outcome + resume outbox。授权不以 bearer 形态存在、不进图状态/checkpoint（与 §8.4 脱敏规则无冲突）；**权威判据唯一 = Ledger 记录**，无签名/HMAC 第二通道。
...
- 原子 compare-and-consume：经 control-plane RPC 在 Ledger 事务内校验授权（有效、未核销、未过期、manifest digest 与 ReleasePlan 规范化结果完全一致）并标记核销；核销绑定 task_id——broker task 中断恢复时"已核销给本 task"视为有效，消除恢复死锁。
- 执行前重核制品/配置 digest（D12）与远端状态（远端幂等，push 前查远端）；不一致 → release_failed，禁止自动 rebase（R9）。
...
### 8.5 Control-plane RPC（特权内部接口）
- 编排进程暴露的唯一状态写入通道：Unix socket + peer credential 认证；Approval 服务、broker、worker 使用**不同 socket、不同 OS 用户与文件权限**——worker socket 仅开放 side_effect/trace 命令，审批与核销命令所在 socket 对 worker 进程不可访问（文件权限隔离）。
- 命令幂等（携带 request_id）；复合状态变更（审批提交、授权核销）为单命令单事务；resume 经 outbox 投递，提交成功而信号丢失时由编排层 outbox 重扫补发。
...
- A3 引用：01/07/08/09（待产出）；docs/review/HLD_v0.1_review_disposition.md、docs/review/HLD_v0.2_review_disposition.md。
