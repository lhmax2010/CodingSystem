...
| `06_Code_Navigation_Evidence_Infrastructure_v0.3.4.md` | v0.3.4 | CNEI（Live 模式，代码导航证据基础设施） | Draft / Spike Required |
| `05_Phased_Development_Plan_v2.1.3.md` | v2.1.3 | Sprint 拆分 + DoD + dev_memory + merge gate | Implementation Plan |
...
| 文档 | 版本 | 用途 |
|---|---|---|
| `MAIN_PROMPT_for_Codex_v2.3.md` | v2.3 | Codex 工作总指南（贯穿 Sprint 0 → M2） |
| `SPRINT_0_PROMPT_for_Codex_v1.2.md` | v1.2 | Sprint 0 Spike Gate 启动专用 |
...
3. `prompts/MAIN_PROMPT_for_Codex_v2.3.md`（工作总指南，必读）
4. `prompts/SPRINT_0_PROMPT_for_Codex_v1.2.md`（Sprint 0 启动）
5. `baseline/00_Agent_Team_Contract_v0.7.3.md`（协议层）
6. `baseline/02_Compiler_Agent_v5.2-RC2.3.md`（Phase 1A 主体）
7. `baseline/06_Code_Navigation_Evidence_Infrastructure_v0.3.4.md`（CNEI）
8. `baseline/05_Phased_Development_Plan_v2.1.3.md`（Sprint 拆分）
9. `adr/ADR_001_CNEI_Scale_Direction.md`（规模化方向，了解 Phase 1A 要预留什么）
10. `design_changes/`（已批准的设计变更，了解 baseline 演进）

### Sprint 1+ 阶段

切换到 `prompts/SPRINT_1_PLUS_PROMPT_for_Codex_v1.3.md`，按 v2.1.3 的 Sprint 任务清单推进。
...
5. **Evidence 失败行为**：degraded 优先，完全无证据才 fail-fast（CNEI v0.3.4 §2.2.4）
...
- CNEI：v0.1 → v0.2 → v0.3.1 → v0.3.2 → v0.3.3 → **v0.3.4**（evidence 失败行为 + 阈值同步）
- Contract：v0.7 → v0.7.1 → v0.7.2 → **v0.7.3**（RawDataDetector 阈值统一）
- 开发计划：v2.0 → v2.1 → v2.1.1 → v2.1.2 → **v2.1.3**（stale 归属 + KI 数量澄清）
- Prompts：MAIN v2.3 / SPRINT_0 v1.2 / SPRINT_1+ v1.3

**最近一次变更**：Codex Sprint 0 design review 发现 6 个 `[DESIGN_ISSUE]`，已在 baseline 修复，记录见 `design_changes/change_1.md`。
