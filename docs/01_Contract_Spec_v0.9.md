...
**版本**: v0.9 Draft（v0.8 三方复审 2 GO / 1 NO-GO 后修订版，待第九轮复审）
**状态**: 复审中
...
**关联**: docs/review/ 下 01 系八轮处置表（57/47/34/21/18/9/9/12 条逐条处置）。本版为**自足文本**：不再以"同 v0.1/v0.2"外引承载任何规范性内容。
...
| C-09 | authorization_consume（broker RPC） | running **∨ publishing（恢复臂）** | 四支见 §2.2a | ①publishing ②closed(按 §2.2b release_failed 映射) ③→C-14 ④publishing（不迁移、不重复核销） |
...
**(a) 核销三支（C-09）**：①授权有效（未核销未撤销未过期、全绑定字段与当前一致）→ 原子核销 + Run→publishing；②授权失效/过期/series 陈旧 → **不核销**，从 running 直接按 (b) 的 release_failed 映射 ledger 级关闭（reason 附 authorization_expired/stale/revoked；即 C-09 目标②）；③D12 digest 不匹配（**核销前检测**）→ 响应臂 d12_mismatch，授权不动，触发 C-14；**④恢复臂（R8-01，HLD §8.3）**：Run=publishing ∧ 授权 consumed_by_task_id == 本 task ∧ 全绑定仍匹配 → 响应 resumed（不重复核销、不迁移状态、可继续发布）；publishing ∧ consumed_by ≠ 本 task → rejected(already_consumed)，fail-closed。
...
class CommandEnvelope(BaseModel): request_id: str; command: CommandName; payload: JsonValue
...
class CommandError(BaseModel):
    ...
    # E-CP-D12-MISMATCH 仅用于 decision_submit / release_dispatch 等非 authorization_consume 的 D12 复核点
class ResponseOk(BaseModel): status: Literal["ok"]; response: JsonValue      # 按命令注册表解析到专属响应模型
class ResponseError(BaseModel): status: Literal["error"]; error: CommandError
...
class AuthorizationConsumeRequest(BaseModel): token_id: str; task_id: str; release_manifest_digest: str
class ConsumeGranted(BaseModel): result: Literal["granted"]        # 已核销，Run→publishing，可发布
class ConsumeRejected(BaseModel): result: Literal["rejected"]; reason: Literal["authorization_expired","stale","revoked","already_consumed"]  # 不核销，Run 已按 §2.2b 关闭
class ConsumeD12Mismatch(BaseModel): result: Literal["d12_mismatch"]  # 授权不动，C-14 已触发（D12 在本命令**仅**以此臂表示，不走 CommandError——X8-01 分工）
class ConsumeResumed(BaseModel): result: Literal["resumed"]          # broker 恢复：已核销给本 task，继续发布（R8-01）
AuthorizationConsumeResponse = Annotated[ConsumeGranted | ConsumeRejected | ConsumeD12Mismatch | ConsumeResumed, Field(discriminator="result")]  # C-09 四支的 wire 形态
class ReleaseOutcomeRequest(BaseModel): task_id: str; outcome: Literal["released","release_failed"]; reason: str = ""; remote_refs: list[str] = []
...
| CF-SM-13 | endpoint_identity 边界 vectors（§5.3 列举） | 装载校验 | 每 negative 拒绝且定位规则；相邻 positive 通过（R8-05） |
...
| CF-AUTH-05 | broker task 中断恢复，Run=publishing、授权 consumed_by=本 task | 再次 authorization_consume | 响应 resumed、不重复核销、状态不迁、继续发布；consumed_by=他 task → rejected(already_consumed) |
...
| CF-IPC-06 | 五方向各一命令（worker / approval / broker 入向 / reconciliation / 编排→broker 出向） | 同载荷重放 / 异载荷同 id / ACK 丢失后重放；heartbeat 窗口内外重放 | 首响应复用 / E-CP-IDEM-MISMATCH / receipt 复用；出向经授权记录状态机去重（granted/resumed/rejected 三臂）；heartbeat 窗口内 mismatch 检测、窗口外视为新 |
...
- **A3 变更记录**：v0.1 初稿；v0.2 = 57 条修订；v0.3 = 47 条修订；v0.4 = 34 条修订；v0.5 = 第四轮 21 条修订；v0.6 = 第五轮 18 条修订；v0.7 = 第六轮 9 条修订；v0.8 = 第七轮 9 条修订；v0.9 = 第八轮 12 条修订（C-09 恢复臂 resumed + 四支响应联合；CommandName 枚举 + 无省略注册表 + Ack/Error 模型 + CommandResponse envelope + D12 单一表示分工；幂等无语义豁免（滚动窗口存储）+ 出向经状态机去重；ProbeReceipt 模型 + 三态 + 保留 digest + 耗尽走能力矩阵；endpoint 自足文法 + inprocess 绑定 + CF-SM-13；编排重启=新 attempt 定案 + CF-SER-11 改写；payload artifact 三元组与提交序）；contract_version 1.0.0.dev1。
