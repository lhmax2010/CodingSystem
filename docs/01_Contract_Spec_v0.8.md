...
**版本**: v0.8 Draft（v0.7 三方复审 2 GO / 1 NO-GO 后修订版，待第八轮复审）
**状态**: 复审中
...
**关联**: docs/review/ 下 01 系五轮处置表（57/47/34/21/18/9/9 条逐条处置）。本版为**自足文本**：不再以"同 v0.1/v0.2"外引承载任何规范性内容。
...
class CandidateRecord(BaseModel):
    ...
    # 分支 B 的 lineage/patch 应用/tree_digest 由此取原文，不依赖进程内缓存（R7-06；重启用例 CF-SER-11）
...
```python
class CommandEnvelope(BaseModel): request_id: str; command: str; payload: JsonValue
# 命令注册表（封闭）：命令名 → 请求模型 → 响应模型；heartbeat→HeartbeatRequest→CommandAck；effect_claim→
# EffectClaimRequest→EffectClaimResponse；effect_transition→EffectTransitionRequest→CommandAck；trace_append→
# TraceAppendRequest→CommandAck；blob_begin→BlobBeginRequest→BlobBeginResponse；blob_commit/abort→…→CommandAck；
# series_candidate_submit→…→SeriesCandidateSubmitResponse；task_complete→TaskCompleteRequest→CommandAck；
# decision_submit→DecisionSubmitCommand→DecisionSubmitResult；authorization_consume→…→AuthorizationConsumeResponse；
# release_outcome→…→CommandAck；release_dispatch→ReleaseDispatch→CommandAck；pending_register→…→CommandAck；
# adjudicate→AdjudicateRequest→AdjudicateResult；post_close_correction→…→CommandAck；probe_report→ProbeReport→CommandAck
...
class ConsumeD12Mismatch(BaseModel): result: Literal["d12_mismatch"]  # 授权不动，C-14 已触发
AuthorizationConsumeResponse = Annotated[ConsumeGranted | ConsumeRejected | ConsumeD12Mismatch, Field(discriminator="result")]  # C-09 三支的 wire 形态（R7-02）
...
# 专用，CC C-vii 定案）。host = dns 名（小写字母/数字/连字符/点，无尾点，IDNA 已 punycode）| IPv4（四段十进制、
# 无前导零）| `[`+RFC 5952 canonical 小写压缩 IPv6+`]`（非压缩形拒绝）；port 仅非 443 时出现、十进制无前导零；
# path 段禁 `.`/`..`/空段、无尾斜杠（根路径省略为空）、禁 query/fragment/userinfo/百分号编码。negative
# vectors（尾点域名/非压缩 IPv6/前导零端口/`:443` 显式/点段路径）随 golden vectors 交付。ProviderExecutionConfig **不含任何 secret 字段**
...
| CF-SER-11 | candidate 登记后编排重启（进程内态清空） | task_complete 分支 B | 自 payload_ref 取回 patch 原文完成 lineage/tree_digest（R7-06） |
...
| CF-IPC-06 | worker/approval/broker 各一命令 | 同载荷重放 / 异载荷同 id / ACK 丢失后重放 | 首响应复用 / E-CP-IDEM-MISMATCH / receipt 复用（R6-01 矩阵） |
...
- **A3 变更记录**：v0.1 初稿；v0.2 = 57 条修订；v0.3 = 47 条修订；v0.4 = 34 条修订；v0.5 = 第四轮 21 条修订；v0.6 = 第五轮 18 条修订；v0.7 = 第六轮 9 条修订；v0.8 = 第七轮 9 条修订（IntEnum/StrEnum 编码分型定案；Envelope 补 command 判别 + 命令注册表 + AuthorizationConsumeResponse 三支联合 + CommandAck/Error + 全方向幂等域 + heartbeat/trace 豁免；已 spawn 无回执合成 indeterminate 有界收敛；endpoint 完整词法文法 + inprocess scheme + negative vectors；自证断言量词限 agent 来源；CandidateRecord.payload_ref 持久化）；contract_version 1.0.0.dev1。
