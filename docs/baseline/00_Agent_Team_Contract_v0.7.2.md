...
- **v0.7.2（本版）**：5.6 Raw Log 补 excerpt 例外条款、8.5.2 L1 redaction 路径局部替换、8.3.1 Skill Phase 1.5 hard-block 计划、2.6.2 Benchmark Phase 1B 启用 disambiguator
...
### 5.6 Raw Log / Raw Data 硬约束（v0.7 新增，v0.7.2 加 excerpt 例外）
...
- `ClineAdapter.call_cline()` 入口必须有 **Raw Data Detector**：
  - **场景 A**（绝对禁止）：检测 prompt 中是否有超过阈值（默认 5000 字符 / 200 行）的连续 raw 风格内容（含 `file:line:` 模式、stack trace、error 关键词密集等特征），且**不在** EvidencePacket.log_excerpt 字段内 → 抛 `RawDataLeakageError`
  - **场景 B**（允许）：< 5000 字符的内容**且**在 EvidencePacket.log_excerpt 字段内**且**符合 5.6.2 约束 → 允许通过
  - 命中场景 A 则落 `raw_data_leakage` FailureEnvelope
