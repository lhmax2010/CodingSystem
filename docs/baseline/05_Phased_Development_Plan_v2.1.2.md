...
- **v2.1.2（本版）**：ChatGPT + Kimi 批次 3 修订 review，consistency cleanup（头部基线版本同步 / Sprint 0 残留 S0-10 清理 / 工时 10.5→11 天 / S0-03 工时 3→3.5 / check_gate.sh 实现细节修正）
...
- 关键设计: [简述]
- **与设计文档对应章节**: [必填，格式：`Compiler v5.2-RC2.2 §A8.3` / `CNEI v0.3.3 §4.3.1` / `Skill v0.2.1 §5.4` 等]（v2.1.1 强化）
- 与原计划的 deviation: [如有，必须解释]
- **本变更涉及的 contract**: [Team Contract v0.7.2 哪些条款？]（v2.1.1 新增）
...
- Compiler Agent v5.2-RC2.2 A1.3 节中的 Phase 1A 时长承诺**同步更新为 3-3.5 个月**
...
| S0-03 | **clangd 启动 + 索引 + 准确率抽样**（v2.1.1 合并）| 3.5 | < 5 min / < 4GB / definition ≥ 90% / references ≥ 85% |
...
| S0-07 | Known Issue matcher 命中/不命中验证 | 1 | 准确率 100%（5 份样例） |
...
- [ ] EvidencePacket Pydantic schema 含所有 v0.3.3 字段
...
- 单元测试覆盖率 ≥ 80%（gate）
- 遵守 Team Contract v0.7.2 + Cognitive Boundary
