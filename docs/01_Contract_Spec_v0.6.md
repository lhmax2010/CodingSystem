...
**版本**: v0.6 Draft（v0.5 三方复审 2 GO / 1 NO-GO 后修订版，待第六轮复审）
**状态**: 复审中
...
**关联**: docs/review/ 下 01 系五轮处置表（57/47/34/21/18 条逐条处置）。本版为**自足文本**：不再以"同 v0.1/v0.2"外引承载任何规范性内容。
...
**candidate 状态机**：registered →（分支 B 消费）consumed /（task 终局未消费或 attempt 重试对账）discarded；series_candidate_submit 幂等按 request_id（同载荷重放返回同 candidate_id），同 task 异载荷二次提交拒绝；attempt 启动对账将上一 attempt 遗留 registered candidate 退役 discarded。
...
# provider_policy_digest = digest(按 provider_id 升序的 ProviderExecutionConfig 列表)；provider_id 全局唯一（重复拒绝）。
# endpoint_identity 规范化算法：scheme/host 小写、缺省端口省略、path 去尾斜杠、禁 query/fragment/userinfo。
# secret 投影为**类型化规则**：模型内 SecretStr 类型字段的值一律替换为固定占位 "<secret>" 后参与 digest
# （非启发式）。端点改向必失配、密钥轮换不失配（CF-SM-12 两用例）
```
...
| CF-FIX-00 | fixture 注册集定案（CF-DSL-FLAG 依赖）：compiler{build_passed,build_failed} / ut{ut_passed,ut_fix_patch_ready} / benchmark{bench_done,bench_failed} / review{pass,concerns,block} / ci{plan_ready}；task_prefix CMP/UT/BEN/REV/CI | 注册 | 两份规范 YAML 的 on/success_on 全部落于该集（V-05/V-14 可判定） |
...
- **A3 变更记录**：v0.1 初稿；v0.2 = 57 条修订；v0.3 = 47 条修订；v0.4 = 34 条修订；v0.5 = 第四轮 21 条修订；v0.6 = 第五轮 18 条修订（HandoffResult series 防伪统一前置校验、candidate 在册数对账判定 + 状态机、AdjudicateRequest/Result 幂等定案、loader plain-scalar 作用域与重复键、ProviderExecutionConfig digest 唯一算法、ProbeDispatch 补全与重放幂等、CF-EF-02 四组合、CF-SER-07..09/CF-AUTH-07/CF-FIX-00、载荷模型补全）；contract_version 1.0.0.dev1。
