...
**关联文档**: 01_Contract_Spec、02–06_Agent_Design_Guide、07_Conformance_and_Onboarding、08_Codex_Dev_Guide、09_Phased_Dev_Plan、docs/review/HLD_v0.1–v0.5_review_disposition.md（v0.5 处置表为本版逐条依据）
...
- **activation 生命周期（退役规则）**：activation_id 由编排层在 **Run 启动的 entry 分派事务（根 activation）**、fan-out、环激活的分派事务内生成并落 Ledger（恢复自 Ledger 重建）；线性边与分支中间节点任务继承派发边的 activation 上下文（传播定义含根 activation，`[01]`）。**supersede 仅两个触发条件**：① series 推进事务同一事务内将旧 series 的全部 open activation 标记 superseded 并作废其计时器；② 某 activation 的已有到达因 series 推进全部转 stale。**同 series 的多个 activation 各自保持 open、独立成组消费、独立超时——不存在"更新者淘汰旧者"规则**（环计数向量无全序，latest-wins 不成立）。到达处理与 timeout handler 均以 CAS 校验该 activation 仍 open 且属 current series——superseded activation 的迟到事件与 timer 一律为审计化 no-op；仅 open 且属 current series 的 activation 超时可路由 fail_safe。model-based 测试覆盖同 series 并发多 activation（§11.3）。
...
- `idem_key = hash(run_id, dispatch_id, effect_class, effect_params_digest, effect_seq)`——**效应身份锚定在 logical effect slot**：slot = (run_id, **dispatch_id**)，dispatch_id 为创建该 task 的边送达/分派事件的 Ledger 标识——**跨 attempt 稳定**（重试复用同一 task 即同一 dispatch）、**跨 task 天然隔离**（同一节点因多次边送达产生的独立 task 各有 dispatch_id，含 entry 分派与同一 fan-out 的兄弟送达，键不碰撞；node/activation/环轮次均由 dispatch 记录蕴含，不再入键）。`effect_params_digest` = side_effect 调用实参的规范化摘要（含所操作的 series_id/tree_digest、构建目标、板卡标识等，artifact 引用按 content digest 参与）；`effect_seq` = 同 slot 内同 (effect_class, effect_params_digest) 的**第 k 次调用**（按计数，非相邻性；序号由编排层在 claim 命令的 Ledger 事务内分配，worker 不本地计数）。**不含 attempt**：参数相同的重试/重放命中同一记录；非决定论重执行实参不同 → 新键，不会错误复用旧效应结果。
- **记录状态与查找矩阵**（全集 claimed / running / succeeded / failed / abandoned / unknown，迁移与查找表 `[01]`）：查找只命中非 abandoned 记录——claimed/running → probe 对账后续跑或补写完成；**succeeded → 直接复用（含上一 attempt 经 probe 确认成功的记录——可 probe 而不可安全重放的效应必须复用，不做保守重做）**；failed → 按效应类别策略重试或上报。attempt 启动前，编排层将上一 attempt 遗留的本 task 非终态记录逐条 probe 对账：确认成功 → 迁移 succeeded（可复用）；未发生 → abandoned；不可判定 → unknown（持久化），同恢复协议报 sys.error。abandoned 记录不参与命中，同参新调用按 effect_seq 递增取新键。
...
- A3 引用：01/07/08/09（待产出）；docs/review/ 下 v0.1–v0.5 五轮处置表。
