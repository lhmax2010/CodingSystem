...
**版本**: v0.10 Draft（v0.9 三方复审 2 GO / 1 NO-GO 后修订版，待第十轮复审）
**状态**: 复审中
...
**关联**: docs/review/ 下 01 系八轮处置表（57/47/34/21/18/9/9/12/8 条逐条处置）。本版为**自足文本**：不再以"同 v0.1/v0.2"外引承载任何规范性内容。
...
| C-09 | authorization_consume（broker RPC） | running **∨ publishing（恢复臂）** | 四支见 §2.2a（③仅 running；④需更高 epoch + 旧执行静默 + 后续远端对账） | ①publishing ②closed(按 §2.2b) ③→C-14 ④publishing（不迁移、不重复核销；对账 indeterminate 经 release_outcome 保持） |
...
**(a) 核销三支（C-09）**：①授权有效（未核销未撤销未过期、全绑定字段与当前一致）→ 原子核销 + Run→publishing；②授权失效/过期/series 陈旧 → **不核销**，从 running 直接按 (b) 的 release_failed 映射 ledger 级关闭（reason 附 authorization_expired/stale/revoked；即 C-09 目标②）；③D12 digest 不匹配（**仅 running、核销前检测**）→ ResponseError(E-CP-D12-MISMATCH)，授权不动，触发 C-14；publishing 期检出漂移 → 不适用 C-14，登记 pending(admin_finalize) 并走 HLD §8.3 对账流程。**④恢复臂（R8-01/R9-01 定案）**：guard = Run=publishing ∧ 授权 consumed_by_task_id == 本 task ∧ 全绑定匹配 ∧ **请求携带的 broker_epoch > 授权记录的 granted_epoch ∧ 旧执行已静默**（编排确认旧 broker 进程组已 reap/lease 过期）→ 响应 resumed(reconcile_required)，授权记录 epoch 更新（不重复核销、不迁移状态）；broker 收到 resumed **必须先远端对账**再动作：confirmed_happened → release_outcome(released)；confirmed_not_happened → 执行发布；indeterminate → release_outcome(indeterminate)，Run 保持 publishing 走 C-11。同 epoch 的第二次 consume（并发/重投）→ rejected(already_consumed)——同 task 不同 request_id 并发恰一成功（CF-AUTH-02 不冲突）。**他 task**：running 下 → rejected 且按 (b) 关闭；publishing 下 → rejected(already_consumed) **仅拒绝、不改 Run 状态**（publishing 只能经 outcome/probe 关闭）。防御：publishing ∧ 本 task ∧ 绑定不匹配（理论不可达）→ ResponseError(E-CP-ILLEGAL-STATE) + 审计（Y9-01）。
...
CommandResponse = Annotated[ResponseOk | ResponseError, Field(discriminator="status")]
...
class AuthorizationConsumeRequest(BaseModel): token_id: str; task_id: str; release_manifest_digest: str; broker_epoch: int   # epoch = 编排为每次 broker 进程启动分配的单调整数
...
class ReleaseOutcomeRequest(BaseModel): task_id: str; outcome: Literal["released","release_failed","indeterminate"]; reason: str = ""; remote_refs: list[str] = []   # indeterminate 仅恢复对账可报，Run 保持 publishing
...
| CF-AUTH-05 | broker 重启（更高 epoch、旧进程组已 reap），Run=publishing、授权 consumed_by=本 task | authorization_consume | resumed(reconcile_required)、epoch 更新、不重复核销、状态不迁；随后远端 probe 三结果：happened→released / not_happened→执行 / indeterminate→release_outcome(indeterminate) 保持 publishing |
...
- **A3 变更记录**：v0.1 初稿；v0.2 = 57 条修订；v0.3 = 47 条修订；v0.4 = 34 条修订；v0.5 = 第四轮 21 条修订；v0.6 = 第五轮 18 条修订；v0.7 = 第六轮 9 条修订；v0.8 = 第七轮 9 条修订；v0.9 = 第八轮 12 条修订；v0.10 = 第九轮 8 条修订（恢复臂绑定 broker_epoch + 旧执行静默 + 远端对账三结果 + 他 task/防御分支出口；D12 回归 HLD 字面唯一 ResponseError 表示；幂等无窗口淘汰——作用域存续期全量 digest 对、Ack 重建；出向 dispatch receipt 绑定；WireJson/JsonValue 双类型 + 投影摘要；响应回显关联；CF-SM-13 canonical 五组复位）；contract_version 1.0.0.dev1。
