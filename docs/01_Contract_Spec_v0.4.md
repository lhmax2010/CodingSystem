...
**版本**: v0.4 Draft（v0.3 三方复审 2 GO / 1 NO-GO 后修订版，待第四轮复审）
**状态**: 复审中
...
**关联**: docs/review/ 下 01 系三轮处置表（57/47/34 条逐条处置）。本版为**自足文本**：不再以"同 v0.1/v0.2"外引承载任何规范性内容。
...
| snapshot_id | `SNAP-` + digest(manifest) 的 hex 前 32 字符（接口引用一律以 snapshot_id 为键；manifest 记录含全量 digest 供审计与碰撞仲裁） | 编排 | 全局 |
...
class RouteSpec(BaseModel):
    ...
    # 模型内部唯一形态为列表。**加载器定案**：YAML 1.2 core schema 语义——实现上以 PyYAML SafeLoader
    # 移除 bool/timestamp 隐式 resolver（`on/off/yes/no/y/n` 一律作字符串）、禁 anchor 合并键；
    # 预处理顺序 = ①按上述 loader 读原文字节 ②value 位置词素化 ③标量 to 列表化 ④pydantic 校验。
    # CF-DSL-FLAG 以旗舰**原文字节**加载断言（`on:` 键必须得字符串键）。
...
- **异参新键允许集合（仅辖跨 attempt 定态记录）**：succeeded/abandoned → 放行（succeeded 仅同参精确键复用、不得跨参复用）；unknown → task 已随 sys.error 中止，无新键。干净终态 failed 依 failed_disposition 处置（§4.3），不在此枚举。
...
class ProbeResult(BaseModel): outcome: ProbeOutcome; result: EffectResult | None  # confirmed_happened 时必填；probe 在**专用 reconciliation worker 子进程**执行（无 Ledger 写权限、复用 worker 数据面隔离与进程组语义；编排进程只据其结果提交状态迁移——R3-08 隔离定案，业务插件代码不入单写者进程）；probe 必须只读、无副作用（conformance 断言）
...
**approval socket**：`decision_submit{ApprovalDecisionRequest}`（R-02；token_id 由编排事务内生成并随响应返回）——release-approval 型事务 = 决定 + 新鲜度 CAS（§2.5）+ 授权记录 + gate CAS + resume outbox（outbox 项含 token_id 供 release dispatch）；plain 型 = 决定 + gate CAS + outbox。
...
| CF-DSL-03..08 | V-03..V-08 每规则独立 a（最小反例）/b（最小正例）行，07 按 rule_id 展开为 12 case | 加载 | 反例拒（定位 rule_id）；正例过 |
...
| CF-EF-05 | 跨 attempt 定态 {succeeded,abandoned,unknown} + 干净终态 failed × 异参新 claim | claim | succeeded/abandoned 放行新键（succeeded 不跨参复用）；unknown 无新键（task 已终止）；**failed 按 failed_disposition/failed_max_retries 展开单元格**（raise → 新键放行但旧记录 error 可查；retry_new_version → 版本链未尽先同键，尽后异参新键放行）——HLD"依类别策略"字面（R3-03） |
...
## 7. 附录
- **A0 类型索引**：本文全部具名类型及定义位置清单（contracts 包按此逐一交付）：§0 SysOutcome/DataClass…；§1 PipelineSpec 族/RetrySpec/预算族；§2 注册与消息族/Report 族/Release 族/ApprovalDecision/AuthorizationRecord/ExecutionManifest/RunContext/TargetSpec 族/SkillError/CreateRunRequest·Response/SubmitPatchRequest·Response/RunStatusResponse/CancelRequest·Response/ResultEnvelope/PendingEvent 联合/RoundDigest/PatchAttribution/Producer 联合/EffectError；§3 ActivationRecord；§4 EffectRecord/EffectResult/ProbeResult/EffectType；§5 命令载荷族/RestrictedOp 族/ProviderPolicy/CorrectionRequest/Record/EvidencePacket 族/TraceEvent（下条）。
...
- **A3 变更记录**：v0.1 初稿；v0.2 = 首轮 57 条原始 issue 全量修订；v0.3 = 次轮 47 条修订；v0.4 = 第三轮 34 条修订（YAML loader 定案、RetryOutcome 收敛、SER 特例清除、Producer 联合、转义表、probe 隔离进 reconciliation worker、token_id 生成方修正、reviewer peer 映射、自足化内联与 §6 展开）；contract_version 1.0.0.dev1。
