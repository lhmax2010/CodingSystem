...
**版本**: v0.7 Draft（v0.6 三方复审 3 NO-GO 后修订版，待第七轮复审）
**状态**: 复审中
...
**关联**: docs/review/ 下 01 系五轮处置表（57/47/34/21/18/9 条逐条处置）。本版为**自足文本**：不再以"同 v0.1/v0.2"外引承载任何规范性内容。
...
class CandidateRecord(BaseModel):
    ...
    request_id: str; payload_digest: str; expected_parent_series_id: str
...
```python
class CommandEnvelope(BaseModel): request_id: str; payload: JsonValue   # payload 按命令名解析到下列模型
...
**特权运维**：`AdjudicateRequest{run_id, request_id, verdict, evidence_refs} → AdjudicateResult{adjudication_id}`（adjudication_id 编排事务内生成——与 §0.2 一致；幂等按 request_id，重放返回同 id 与同结果）——guard：Run=publishing ∧ 强制 probe 后 unknown ∧（verdict∈confirm_* ⇒ evidence_refs 非空）∧（confirm_not_released ⇒ broker task 静默已确认）；`post_close_correction{run_id, CorrectionRequest}`。
...
**通用 RPC envelope（R6-01，HLD §8.5"命令幂等"的统一落地）**：全部 control-plane 命令经 `CommandEnvelope{request_id, payload}` 提交；request_id 由调用方进程生成；幂等域按 socket 角色——worker 数据面 = (task_id, attempt, 命令名)、approval = (approval 服务, 命令名)、broker = (broker, 命令名)、特权运维 = (运维 principal, 命令名)；保留期 = Run 存续 + 7 天；受理即持久化 receipt（request_id → payload digest + 首响应），**同 id 同载荷重放返回首响应（事务后 ACK 丢失安全）、同 id 异载荷 → E-CP-IDEM-MISMATCH**；复合变更单命令单事务；outbox/inbox 去重同 HLD §4.4。重放矩阵用例 CF-IPC-06。
...
# （ASCII 封闭——字节序与码点序一致）、全局唯一（重复拒绝）。endpoint_identity **在配置装载时按封闭文法校验**
# （不做运行时变换）：scheme 仅 https、host 小写（IDNA 域名须已 punycode、IPv6 用小写方括号字面量）、
# 禁缺省端口 443 显式出现、path 无尾斜杠（根路径省略为空）、禁 query/fragment/userinfo/百分号编码——
# 非 canonical 输入直接拒绝，规范化即校验、无实现自由度。ProviderExecutionConfig **不含任何 secret 字段**
...
- **A3 变更记录**：v0.1 初稿；v0.2 = 57 条修订；v0.3 = 47 条修订；v0.4 = 34 条修订；v0.5 = 第四轮 21 条修订；v0.6 = 第五轮 18 条修订；v0.7 = 第六轮 9 条修订（CF-FIX-00 注册集逐名对齐旗舰原文——三家同发现；control-plane 统一 CommandEnvelope 与分角色幂等域；CandidateRecord 跨 attempt 恢复定案；probe receipt 判定序与 pending 重派；provider digest 文法封闭 + golden vector；命令载荷全模型化）；contract_version 1.0.0.dev1。
