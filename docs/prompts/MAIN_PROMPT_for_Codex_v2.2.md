...
**版本**：v2.2（对应文档基线 v0.7.2 / RC2.2 / RC2.3 / v0.3.3 / v2.1.2 / v0.2.1 / v0.3）
...
- 关联基线版本同步到 v2.1.2 / v0.3（开发计划 + Phase 1.5 + Demo）
...
**协议层**：
1. `00_Agent_Team_Contract_v0.7.2.md` ← **Locked，不会变**

**Agent 设计层**：
2. `02_Compiler_Agent_v5.2-RC2.2.md` ← Phase 1A 主体
3. `03_Benchmark_Agent_v5.2-RC2.3.md` ← Phase 1B 主体
   （注：Compiler 是 RC2.2，Benchmark 是 RC2.3——RC2.3 修订仅针对 Benchmark 的 B8.2 analysis None 断言）

**共享基础设施**：
4. `06_Code_Navigation_Evidence_Infrastructure_v0.3.3.md` ← CNEI

**实施计划层**：
5. `05_Phased_Development_Plan_v2.1.2.md` ← Sprint 拆分 + DoD + dev_memory + merge gate
...
### 2.1 Sprint 0 任务清单（见 v2.1.2 §1.2.1）
...
完整任务列表见《开发计划 v2.1.2》§Sprint 0 详细规划 + 《SPRINT_0_PROMPT v1.1》§2。
...
7. 写 spike_report 用 v2.1.2 §0.3 / Compiler A18.2 的模板
...
9. doc consistency warning（v2.1.2 §0.3.1 第 9 项）
...
## 2. 设计决策
- 关键设计: [简述]
- **与设计文档对应章节**: [必填，格式如 `Compiler v5.2-RC2.2 §A8.3`]
- 与原计划的 deviation: [如有]
- **本变更涉及的 contract**: [Team Contract v0.7.2 哪些条款？]
...
- 抽样 reference 时发现 clangd 对某些 macro-heavy file 解析慢，
  单次 reference query 5-8 秒，超过 v0.3.3 规定的 10s timeout 边界
  → 选项：
