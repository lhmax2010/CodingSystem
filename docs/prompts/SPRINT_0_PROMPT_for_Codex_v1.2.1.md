# Sprint 0 Spike Gate 启动 Prompt v1.2.1（Phase 1A）
...
**v1.2 修订摘要**（Codex Sprint 0 design review Issue 1）：

- stale 检测在 S0-03 和 S0-09 重复出现导致归属不清 — S0-03 改为"mtime 基础探测"，完整 stale 检测 + confidence 降级验证归 S0-09（独立 core gate）
- 关联文档版本同步：Compiler RC2.3 / Benchmark RC2.4 / CNEI v0.3.4 / Contract v0.7.3 / 开发计划 v2.1.3

**v1.2.1 修订摘要**（Codex S0-01 反馈）：

- S0-01 "至少 3 个开发者熟悉 repo" 原是 dogfooding 软偏好，对 spike 偏严 — 拆分为：spike 硬标准（1-2 人能做 ground truth 核对）+ dogfooding 软偏好（非阻塞，Phase 1A 启动时可另选 dogfooding 目标）
...
- [ ] **ground truth 可核对**（spike 硬标准）：至少 1-2 个能读 C/C++/CMake 的人可对 clangd 准确率、错误分类做人工核对（repo 代码量小即可，不需资深 owner）
- [ ] （软偏好，非阻塞）有开发者熟悉该 repo 便于后续 dogfooding —— 若 Phase 1A 正式开发时该 repo 无人熟悉，可在 Phase 1A 启动时另选 dogfooding 目标，不影响 Sprint 0 spike
...
- [ ] **stale 基础探测**：确认 clangd 能感知 compile_commands.json 的 mtime（完整 stale 检测 + confidence 降级验证在 S0-09，此处只验证 mtime 可读）
...
- mtime 基础探测结果（完整 stale 验证见 S0-09）
